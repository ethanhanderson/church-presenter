...
import { CursorLayer, CursorProvider } from '@/components/cursor';
...
export default function App() {
  ...
  const { selectedSlideId, selectedSlideSource, setSelectedSlideId } = useShowStore();
  ...
  const [themeConflictData, setThemeConflictData] = useState<ThemeConflictData | null>(null);
  const [themeConflictDialogOpen, setThemeConflictDialogOpen] = useState(false);

  // Auto-save hook - waits for edits to settle to keep UI responsive
  ...
  useThemeAutoSave({
    debounceMs: 1000,
    enabled: activePage === 'themes',
    onConflict: (conflict) => {
      setThemeConflictData(conflict);
      setThemeConflictDialogOpen(true);
    },
    onError: (error) => {
      console.error('Theme auto-save error:', error);
    },
  });
  ...
  const handleResolveThemeConflict = useCallback(async (choice: 'local' | 'remote') => {
    if (!themeConflictData) return;

    try {
      await resolveThemeConflict(choice, themeConflictData);
      setThemeConflictData(null);
    } catch (error) {
      console.error('Failed to resolve theme conflict:', error);
    }
  }, [themeConflictData]);
  ...
  const outputWindowStateRef = useRef<{ enabled: boolean; configuredKey: string }>({
    enabled: false,
    configuredKey: '',
  });
    ...
    const initialize = async () => {
      try {
        ...
        if (storedSettings.contentDir && storedSettings.contentDir !== currentContentDir) {
          // Persisted settings may still reference an old Documents path; only apply if it is
          // under the repo content root (same rule as set_content_dir in Rust).
          if (await isContentDirUnderRepo(storedSettings.contentDir)) {
  ...
  useEffect(() => {
    ...
    const unlisten = listen('output:disable-audience', () => {
      updateSettings({
        output: { ...settings.output, audienceEnabled: false },
      });
    });
  ...
  }, [settings.output, updateSettings]);
  ...
  useEffect(() => {
    ...
    const normalizedConfigured = [...configured].sort((a, b) => a - b);
    const configuredKey = normalizedConfigured.join(',');
    if (outputState.enabled && outputState.configuredKey === configuredKey) {
      return;
    }

    outputState.enabled = true;
    outputState.configuredKey = configuredKey;
    void openOutputWindows(normalizedConfigured);
  ...
  useEffect(() => {
    ...
    if (!hasSlide) return;
    if (currentSlideId === selectedSlideId) return;
    goToSlide(selectedSlideId);
  }, [isLive, livePresentation, selectedSlideId, selectedSlideSource, currentSlideId, goToSlide]);
  ...
  const moveShowSelection = useCallback(
    (direction: 'next' | 'previous') => {
      ...
      if (!selectedSlideId) {
        setSelectedSlideId(presentation.slides[0]?.id || null, 'user');
        return;
      }
      ...
      const nextIndex =
        direction === 'next'
          ...
          : Math.max(index - 1, 0);
      setSelectedSlideId(presentation.slides[nextIndex]?.id || null, 'user');
    },
  ...
  return (
    <CursorProvider>
      <TooltipProvider>
        ...
        />
        <ThemeSaveConflictDialog
          open={themeConflictDialogOpen}
          onOpenChange={setThemeConflictDialogOpen}
          conflict={themeConflictData}
          onResolve={handleResolveThemeConflict}
        />
        <UpdateDialog
