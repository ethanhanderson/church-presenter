...
import { useSettingsStore, useEditorStore, useThemeStore, useWorkspaceStore } from '@/lib/stores';
...
export function AppMenubar({
  ...
  const { autoSave: themeAutoSave, isDirty: isThemeDirty } = useThemeStore();
  const { activePage } = useWorkspaceStore();
  const effectiveAutoSave = activePage === 'themes' ? themeAutoSave : autoSave;
  const effectiveIsDirty = activePage === 'themes' ? isThemeDirty : isDirty;
  const showAutoSave = activePage === 'themes' || !!presentation;
      ...
      <div className="ml-auto flex items-center gap-2">
        {showAutoSave && (
          <AutoSaveStatusIndicator
            status={effectiveAutoSave.status}
            isDirty={effectiveIsDirty}
            lastSaved={effectiveAutoSave.lastSaved}
            error={effectiveAutoSave.lastError}
          />
        )}
      </div>
    </Menubar>
