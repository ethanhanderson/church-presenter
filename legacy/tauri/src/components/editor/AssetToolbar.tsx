...
import type { MediaCueTarget, MediaEntry, SlideMediaCue } from '@/lib/models';
import { useEditorAssetToolbarStore, type AssetToolbarStore } from '@/lib/editor/slide-surface-store';
...
export function AssetToolbar({
  ...
  const editorStore = useEditorAssetToolbarStore();
  const {
    presentation,
    addTextLayer,
    addShapeLayer,
  ...
  const disableNonTextLayers = false;
  const canImportMedia = allowMediaImport && typeof addMedia === 'function';

  const [webUrl, setWebUrl] = useState('');
  const [webPopoverOpen, setWebPopoverOpen] = useState(false);
  const [mediaPopoverOpen, setMediaPopoverOpen] = useState(false);
  const defaultMediaTarget: MediaCueTarget = 'mediaUnderlay';
  ...
  const handleAddMedia = (media: MediaEntry) => {
    ...
    const target = media.type === 'audio' ? 'audio' : defaultMediaTarget;
    const nextCue: SlideMediaCue = {
      id: crypto.randomUUID(),
      mediaId: media.id,
      mediaType: media.type,
      target,
      fit: media.type === 'audio' ? undefined : 'cover',
      loop: media.type === 'video',
      muted: media.type === 'video',
      autoplay: media.type !== 'image',
    };

    updateSlide(slideId, { mediaCues: [...(slide.mediaCues ?? []), nextCue] });
  ...
  return (
    <div className="flex items-center justify-center gap-1 px-3 py-2 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/80 border border-border/70 rounded-xl">
      {/* Text */}
      <DropdownMenu>
        <Tooltip>
