...
if (import.meta.env.DEV && typeof performance !== 'undefined') {
  ...
  if (originalMeasure) {
    performance.measure = (
      ...
      try {
        return originalMeasure(name, startOrOptions as never, end as never);
      } catch (error) {
        if (error instanceof DOMException && error.name === 'DataCloneError') {
          if (startOrOptions && typeof startOrOptions === 'object') {
            const { start, end: optionsEnd } = startOrOptions as PerformanceMeasureOptions;
            return originalMeasure(name, start, optionsEnd);
          }
          return undefined;
        }
        throw error;
