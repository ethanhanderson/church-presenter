...
/// <summary>Creates and owns fullscreen stage windows from the configured logical output topology.</summary>
public sealed class StageWindowService(
    IOutputTopologyService topology,
    IServiceProvider services,
    ILiveProductionFacade liveProduction,
    ILogger<StageWindowService> logger) : IStageWindowService, IDisposable
{
    ...
    private readonly ILogger<StageWindowService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public void Open()
    {
        ...
        RunOnUiThread(() =>
        {
            ...
            IReadOnlyList<LocalDisplayOutputTarget> requestedTargets = topology.GetLocalDisplayTargets(OutputScreenIds.Stage);
            ...
            {
                OutputScreenDiagnostics diagnostics = topology.ResolveDiagnostics(OutputScreenIds.Stage);
                _logger.LogWarning("Stage output has no mapped local display endpoints. {DiagnosticsMessage}", diagnostics.Message);
                CloseAllCore();
                return;
            }
            ...
            {
                OutputScreenDiagnostics diagnostics = topology.ResolveDiagnostics(OutputScreenIds.Stage);
                _logger.LogWarning("Stage output targets are unavailable. {DiagnosticsMessage}", diagnostics.Message);
                CloseAllCore();
                return;
            }
    ...
    private AudienceOutputWindow GetOrCreateWindow(LocalDisplayOutputTarget target)
    {
        ...
        var outputPage = _services.GetRequiredService<StageOutputPage>();
    ...
    private void CloseWindow(OutputWindowTrackingKey key)
    {
        ...
        {
            _logger.LogDebug(ex, "Failed to close stage output window for endpoint {EndpointId}.", key.EndpointId);
            window.Dispose();
        }
    }
        ...
        int? monitorIndex = null)
    {
        _liveProduction.ReportOutputHostFeedback(new OutputHostFrameFeedbackState
        {
            ...
            Detail = isVisible ? "Stage output window is visible." : "Stage output window is hidden.",
        });
    }
